...
namespace souffle {
  static
  const RamDomain RAM_BIT_SHIFT_MASK = RAM_DOMAIN_SIZE - 1;
  struct t_btree_ii__0_1__11__10 {
    static constexpr Relation::arity_type Arity = 2;
    using t_tuple = Tuple < RamDomain, 2 > ;
    struct t_comparator_0 {
      int operator()(const t_tuple & a,
        const t_tuple & b) const {
    ...
    bool insert(const RamDomain * ramDomain) {
      ...
      const t_tuple & tuple = reinterpret_cast <
        const t_tuple & > (data);
      context h;
    ...
    range < iterator > lowerUpperRange_00(const t_tuple & /* lower */ ,
      const t_tuple & /* upper */ , context & /* h */ ) const {
  ...
  class Sf_first: public SouffleProgram {
    private: static inline std::string substr_wrapper(const std::string & str, std::size_t idx, std::size_t len) {
      std::string result;
      try {
    ...
    public: Sf_first(): wrapper_rel_1_edge(0, * rel_1_edge, * this, "edge", std::array <
      const char * , 2 > {
          {
            "i:number",
            "i:number"
    ...
    wrapper_rel_2_path(1, * rel_2_path, * this, "path", std::array <
      const char * , 2 > {
          {
            "i:number",
            "i:number"
    ...
    private: std::string inputDirectory;
    ...
    public: void runAll(std::string inputDirectoryArg = "", std::string outputDirectoryArg = "", bool performIOArg = true, bool pruneImdtRelsArg = true) override {
      runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
    }
    public: void printAll(std::string outputDirectoryArg = "") override {
      try {
        std::map < std::string, std::string > directiveMap({
          ...
          {
            "params",
            "{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"
          },
          {
            "types",
            "{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"
          }
        });
    ...
    public: void loadAll(std::string inputDirectoryArg = "") override {
      try {
        std::map < std::string, std::string > directiveMap({
          {
            "IO",
    ...
    public: void dumpInputs() override {
      try {
    ...
    public: void dumpOutputs() override {
      try {
        std::map < std::string, std::string > rwOperation;
        rwOperation["IO"] = "stdout";
    ...
    void executeSubroutine(std::string name,
      const std::vector < RamDomain > & args, std::vector < RamDomain > & ret) override {
      if (name == "stratum_0") {
        subroutine_0(args, ret);
        return;
      }
  ...
  SouffleProgram * newInstance_first() {
    return new Sf_first;
  }
  SymbolTable * getST_first(SouffleProgram * p) {
    return &reinterpret_cast < Sf_first * > (p) -> getSymbolTable();
  }

  #ifdef __EMBEDDED_SOUFFLE__
  class factory_Sf_first: public souffle::ProgramFactory {
    SouffleProgram * newInstance() {
      return new Sf_first();
    };
    public:
      factory_Sf_first(): ProgramFactory("first") {}
  };
  extern "C" {
    factory_Sf_first __factory_Sf_first_instance;
  }
}
#else
}
